...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 3 {
        eprintln!("Usage: {} -[ic] <path-to-binary>", args[0]);
        std::process::exit(1);
    }

    if args[1] != "-i" && args[1] != "-c" {
...
fn analyze_connection(output: &str) -> Result<(), Box<dyn std::error::Error>> {
    let mut functions: Vec<(String, Vec<String>)> = Vec::new();
    let mut current_function: Option<String> = None;
    let function_regex = Regex::new(r"([0-9a-f]+) <(.+?)>:")?;
    let child_regex = Regex::new(r"([0-9a-f]+): .+ <(.+?)>")?;
    let mut children: Vec<String> = Vec::new();

    for line in output.lines() {
        if let Some(captures) = function_regex.captures(line) {
            if let Some(func) = current_function.replace(captures[2].to_string()) {
                functions.push((func, children.clone()));
                children.clear();
                continue;
            }
            ...
            if let Some(child) = child_regex.captures(line) {
                children.push(child[2].to_string().clone());
            }
    ...
    if let Some(func) = current_function {
        functions.push((func, children));
    }

    for (function, children) in functions {
        println!("{}:", function);
